...
\begin{longtable}[c]{cccc}
	\caption[título optativo de la tabla]{Propiedades de los espacios $L^p$.}\label{tablaLp}\\
	\hline
	...
	\caption*{Fuente: tomada de \cite[Cap. 3]{Brez}.}\vspace{-12pt}
\end{longtable}
...
$X$ al conjunto $Y$, es decir $f\colon X\To Y$. Para cualquier subconjunto
$T\subseteq Y$, definimos la \textbf{imagen inversa} de $T$, bajo
$f$, denotada por $\ff (T)$, como sigue:
\[\ff (T)=\{s\in X\mid f(s)\in T\}.\]\end{defn} 

\begin{thm}\label{thcp1}Para la aplicación $\ff \colon {\cal{P}} (Y)
...
\[\Big(\sup_j f_j\Big)(x)\df \sup_j(f_j(x)),\quad \Big(\limsup
\limits_{j \rightarrow \infty} f_j\Big)(x)\df \limsup \limits_{j
...
\includegraphics[width=0.8\linewidth]{analisisGraficoModelo3}\\
...
\begin{defn}\label{dfcp5}Sea $f\colon X\To \RR$ una aplicación. Se definen
...
\begin{prp}\label{prcp2}Para cualquier aplicación $f\colon X\To \R$
...
\begin{longtable}[c]{| c | c |}
	...
	\hline\hline
	\caption*{Fuente: La tabla se generó a partir del código dado en  \url{https://www.overleaf.com/learn/latex/tables} con ligeros cambios.}\vspace{-6pt}
	\endlastfoot
