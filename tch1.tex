...
\subsection{Tablas}
	...
	\caption{Funciones especiales}\label{t:div}\\ \hline
	Salida & Comando & Salida & Comando \\ \hline \endfirsthead
	\multicolumn{4}{l}{\textit{(viene en la anterior página)}}\\
	\hline
	Salida & Comando & Salida & Comando \\ \hline \endhead
	\hline \multicolumn{4}{l}{\textit{(continúa en la página siguiente)}} \\ \endfoot
	\hline \endlastfoot
	$\arg$ & \verb"\arg" & $\cos$ & \verb"\cos" \\
	$\cosh$ & \verb"\cosh" & $\cot$ & \verb"\cot" \\
	$\coth$ & \verb"\coth" & $\csc$ & \verb"\csc" \\
	$\deg$ & \verb"\deg" & $\det$ & \verb"\det" \\
	$\dim$ & \verb"\dim" & $\exp$ & \verb"\exp" \\
	$\gcd$ & \verb"\gcd" & $\hom$ & \verb"\hom" \\
	$\inf$ & \verb"\inf" & $\ker$ & \verb"\ker" \\
	$\lg$ & \verb"\lg" & $\ln$ & \verb"\ln" \\
	$\log$ & \verb"\log" & $\max$ & \verb"\max" \\
	$\min$ & \verb"\min" & $\Pr$ & \verb"\Pr" \\
	$\sec$ & \verb"\sec" & $\sin$ & \verb"\sin" \\
	$\sinh$ & \verb"\sinh" & $\sup$ & \verb"\sup" \\
	$\tan$ & \verb"\tan" & $\tanh$ & \verb"\tanh" \\
	$\arccos$ & \verb"\arccos" & $\arcsin$ & \verb"\arcsin" \\
	$\arctan$ & \verb"\arctan" & $\lim$ & \verb"\lim" \\
	$\liminf$ & \verb"\liminf" & $\limsup$ & \verb"\limsup" \\
	$\bmod$ & \verb"\bmod" & $\pmod$ & \verb"\pmod" \\
	$\mod$ & \verb"\mod" & $\sen$ & \verb"\sen"\\ [3pt]
	\hline
\end{longtable} Notemos que \verb"\sen" se reconoce como seno, por lo cual es la opción a utilizar en español.
